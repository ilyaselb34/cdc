...
def verif_dup(data: pd.DataFrame, ind_step: int, wanted_step: int):
    """Verify if there is a similar date in the data and if the measures can be
    duplicated. Similar dates are dates that are 7 days apart with a limit of
    3 weeks

    Args:
        data (pd.DataFrame): pandas Dataframe with the data to check,
    ...
    Returns:
        ...
        date (datetime): the substitute date if there is one, the same date
            with ind_step as index otherwise.
    """

    """We take the date of the irregular step and we look for a similar date in
    the data"""
    ...
    while i <= 3 and not flag:
        ...
        """If the date is found, we store it in the variable date and put an
        end to the loop"""
        if include_1:
            date = date_1
            flag = True
    ...
    """We check if the data is duplicable"""
...
def data_duplication(data: pd.DataFrame, ind_step: int, wanted_step: int):
    ...
    """These variables are used to calculate the number of weeks between the
    wanted date and the substitute date"""
    step = data['time_step'][ind_step]
    cumul_step = 0
    weeks = abs((date - data['date'][ind_step]).total_seconds()
                / 60 / 60 / 24 / 7)
    ...
    if flag:
        ...
        """We duplicate the data depending if the date we found is before or
        after the irregular step"""
        while index > 0 and cumul_step < step:
            if date > data['date'][ind_step]:
                res.loc[len(res)] = [
                    data['date'][index] - dt.timedelta(days=7 * weeks),
                    data['power'][index]
                ]
            else:
                res.loc[len(res)] = [
...
def dataset_correction(data: pd.DataFrame, wanted_step: int):
    ...
    for i in range(1, len(res)):
        if res['time_step'][i] >= 240:
            res2 = pd.concat([res2, data_duplication(res, i, wanted_step)],
                            ignore_index=True)
    res = pd.concat([res, res2], ignore_index=True)
    res = res.sort_values(by='date').reset_index(drop=True)
    time_step = (res['date'].diff() / pd.Timedelta(minutes=1)).fillna(0)
    res = res[res['time_step'] != 0.0]
    del res['time_step']"""
    del data['time_step']

    return res

print(verif_dup(data, 363, 60))
...
print(step_change)
data_corrected = data_corrected[data_corrected['time_step'] != 0]
