...
def lin_interpolation(data: pd.DataFrame, ind_step:int, wanted_step: int):
    ...
    # Adds data points to the new dataframe, adapting the power value to the time step
...
def dataset_lin_interpolation(data: pd.DataFrame, wanted_step: int):
    ...
    # Adds a new column to the dataframe with the time step between each data point
    time_step = (data['date'].diff() / pd.Timedelta(minutes=1)).fillna(0)
    data['time_step'] = [0]+time_step
    
    # Interpolates the data points with a time step bigger than the wanted time step
